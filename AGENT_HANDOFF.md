...
1. **Kiosk proxy prefers the box's /tts/prompt cache, ADR-007 (2026-06-11).**
...
2. **Pokémon name pronunciation + Memory Match replay music leak (2026-06-11).**
...
3. **Lock-screen lockout fixed in kiosk-lockdown.sh (2026-06-11).** The
...
4. **Creative corner modules (2026-06-11, ADR-006).** Four new frontend-only
...
5. **Voice narration + menu-only music (2026-06-09, ADR-005).**
...
6. **Time-based blocking removed end-to-end (2026-06-09, ADR-004).**
...
7. **Number-only math for Michael** (`d9a1262`).
...
8. **Default avatars changed** (`33d3608`).
...
9. **Pokédex/habitat support extended** — Zapdos (stormy mountain sky) and
   Dracovish (rocky ocean shore) in `src/lib/pokemonHabitat.ts` (rendered with
   the generic mountain/ocean GeoScenes).

10. **Security/test work** — API tests cover unauthenticated admin/profile
