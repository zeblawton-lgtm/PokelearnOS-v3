...
function spriteClass(total: number) {
  if (total <= 5) return "w-16 h-16";
  if (total <= 10) return "w-12 h-12";
  if (total <= 15) return "w-9 h-9";
  return "w-7 h-7";
}
...
function QuestionVisual({
  ...
  return <img src={SPRITE(q.pokemonId)} alt={pokemonName} className="w-24 h-24 object-contain mx-auto drop-shadow-lg" />;
}
      ...
      <div className="flex flex-col items-center justify-center h-full px-6 text-center">
        ...
        >
          <img
            ...
            className="w-40 h-40 mx-auto mb-4 drop-shadow-xl"
          />
